module ViewComponentUI
  class TabsComponent < PaperComponent
    ...
    default_props x_data: proc { POINTER_METHOD_PROPS[props[:pointer_method]][:tabs][:x_data] }
    ...
    class TabComponent < ActiveLinkComponent
      ...
      default_props as: :a, display: :inline_block, cursor: :pointer, px: 4, py: 2, rounded: true, _hover: { color: 'blue-600' },
               on_click: proc { POINTER_METHOD_PROPS[props[:pointer_method]][:tab][:on_click] },
    ...
    class TabPaneComponent < BoxComponent
      default_props x_show: proc { POINTER_METHOD_PROPS[props[:pointer_method]][:tab_pane][:x_show] }
